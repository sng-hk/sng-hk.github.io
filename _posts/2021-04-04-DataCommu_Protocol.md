...
### 통신 프로토콜의 예  
(절차)  송신자 -> 수신자 : 데이터를 송신하려고 하는데 수신 준비가 됐는가?  
(절차)  송신자 <- 수신자 : 수신 준비가 됐다.  
(데이터)송신자 -> 수신자 : 파일을 첨부한 **전자우편**을 보냈다.  
(절차)  송신자 <- 수신자 : 올바르게 수신했다.  
...
국제 표준화기구에서 **OSI 참조 모델**을 제정하여 발표.  
**현재는 TCP/IP**를 많이 사용.  
...
**데이터를 네트워크를 통해서 전송할 떄 일정한 품질을 제공해줄 수 있는 기능.**  
* **5 계층 : Session Layer**  
**문서의 전송 방향과 절차 확인**  
* **6 계층 : Presentation Layer**  
**사용자의 컴퓨터 시스템마다 운영체제, 데이터 표현 방식, 컴퓨터 하드웨어가 달라서**  
**주고받는 데이터가 다를수 있기 때문에 이것을 맞춰주는게 기능**  
* **7 계층 : Application Layer(최상위 계층)**  
**파일을 보내거나 이메일을 보내는 기능.**  
  
## OSI 7 Layer 계층 상호 간의 데이터 전달 원리  
보고서나 편지를 보낸다고 할 때, 편지의 내용은 data에 해당.  
표지, 주소는 데이터에 해당하지 않음.누가 썼는지에 대한 정보이다.즉, 데이터에 대한 정보.  
이것을 헤더라고 부른다.  
송신측 최상위 계층(응용 계층)에서 발생한 데이터를 하위 계층으로 차례로 전달.  
처음의 데이터에 각 계층에 전달받은 헤더정보를 추가하여 전달  
최하위 계층(물리 계층)에 도달.  
수신측은 각 계층의 헤더에 해당되는 부분을 벗긴 후 최상위 계층으로 전달.  
송신 측에서 각 계층마다 자기 기능에 대한 헤더를 붙인다.  
